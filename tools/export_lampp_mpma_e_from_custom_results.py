...
def _build_lampp_level_arrays(
    ...
    meta = set(metadata_cols or LAMPP_META_COLS) | {sample_id_col, target_col}
...
def main():
    ...
    p.add_argument(
        "--experiment-dir",
        type=Path,
        default=Path("FINAL/experiments/SCZ-LAMPP-NCV"),
        help="Existing LAMPP experiment directory.",
    )
    p.add_argument(
        "--csv",
        type=Path,
        default=Path("data/lampp-newest/data/scz/scz_train.csv"),
        help="Full labelled wide CSV used by the benchmark sweep.",
    )
    p.add_argument(
        "--out",
        type=Path,
    ...
    p.add_argument("--model-id", default="scz_mpma_e", help="Stable backend model id.")
    p.add_argument(
        "--display-name",
        default="SCZ MPMA-E",
    ...
    p.add_argument(
        "--target", default="SCZ", help="Short target label for model lists."
    )
    p.add_argument(
        "--positive-value",
        default="1",
