...
def _read_selected_unit(experiment_dir: Path) -> dict[str, Any]:
    ...
    # Historical LAMPP/LODO exporters used two nearly identical keys:
    #   - inner_val_best_ensemble          (CRC/GHS generic LODO scripts)
    #   - inner_val_best_mpmas_ensemble    (IBD 2-cohort named-config script)
    block = data.get("inner_val_best_ensemble")
    if not isinstance(block, dict):
        block = data.get("inner_val_best_mpmas_ensemble")
    if not isinstance(block, dict):
        raise ValueError(
            f"{path} must contain inner_val_best_ensemble or "
            "inner_val_best_mpmas_ensemble"
        )
    members = block.get("members")
    if not isinstance(members, list) or not members:
        raise ValueError(f"{path}: selected ensemble members are empty or invalid")

    # Normalize for the rest of the exporter.
    data["inner_val_best_ensemble"] = block
    return data
...
def _filter_lampp_studies(
    ...
    if out.empty:
        raise ValueError(
            ...
            f"Check --study-ids against the IBD training CSV."
        )
    return out
...
def _manuscript_model_factory(model_name: str, module=None):
    ...
    # Linear / ridge / SGD / PA
    if name in {"LR", "LR_default", "LR_C1"}:
    ...
    if name.startswith("LR_en_l"):
        # Examples: LR_en_l025, LR_en_l050, LR_en_l050_C01, LR_en_l075_C1_bal.
        # Older inactive IBD registries used unsuffixed names; those mean C=1.0.
        import re
    ...
    if name in {"Bag_LR", "Bag_LR_C01"}:
        # Legacy/inactive configs can appear when the IBD ensemble is run with
        # --include-inactive-configs. Older registries used the unsuffixed
        # Bag_LR name for bagged C=0.1 logistic regression.
        return _bagging_classifier(
    ...
    if name.startswith("Bag_"):
        # General legacy bagging fallback for inactive registry rows selected by
        # the IBD include-inactive ensemble. Examples observed in completed
        # IBD configs include Bag_PA_sq.  Use the underlying factory to keep
        # aliases such as PA_sq_C01, SGD_log, ET_default, etc. consistent.
        base_name = name[len("Bag_") :]
        n_estimators = 50
        if "_" in base_name:
            maybe_base, maybe_n = base_name.rsplit("_", 1)
            if maybe_n.isdigit():
                base_name = maybe_base
                n_estimators = int(maybe_n)

        if base_name in {"LR", "LR_C01"}:
            base_estimator = LogisticRegression(C=0.1, max_iter=1000, random_state=42)
        else:
            try:
                base_estimator = _manuscript_model_factory(base_name, module=module)
            except KeyError:
                base_estimator = None
...
def _factory_preflight_report(
    configs: pd.DataFrame, module=None, scope: str = "selected"
) -> pd.DataFrame:
    """Return one row per unique model name with factory status.

    This deliberately checks *all* unique model names in the provided dataframe
    before any expensive final fitting is started, so missing legacy aliases are
    reported as a complete list rather than one-at-a-time during training.
    """
    if "model" not in configs.columns:
        raise ValueError("Config dataframe has no 'model' column for factory preflight")

    names = sorted(
        {str(x) for x in configs["model"].astype(str).tolist() if str(x).strip()}
    )
    rows: list[dict[str, Any]] = []
    for name in names:
        try:
            estimator = _make_model(name, module=module)
            rows.append(
                {
                    "scope": scope,
                    "model": name,
                    "ok": True,
                    "factory_class": estimator.__class__.__name__,
                    "error_type": "",
                    "error": "",
                }
            )
    ...
    return pd.DataFrame(
        rows, columns=["scope", "model", "ok", "factory_class", "error_type", "error"]
    )


def _print_factory_preflight_report(report: pd.DataFrame, scope_label: str) -> None:
    n_total = int(len(report))
    n_ok = int(report["ok"].sum()) if n_total else 0
    n_missing = n_total - n_ok
    print(
        f"Factory preflight [{scope_label}]: {n_ok}/{n_total} unique model type(s) can be instantiated.",
        flush=True,
    )
    if n_missing:
        print(f"Missing factories [{scope_label}] ({n_missing}):", flush=True)
        for _, row in report.loc[~report["ok"]].sort_values("model").iterrows():
            print(
                f"  - {row['model']}: {row['error_type']}: {row['error']}", flush=True
            )


def _preflight_selected_model_factories(
    selected_configs: pd.DataFrame, module=None
) -> pd.DataFrame:
    """Validate all selected model factories before expensive final training starts."""
    report = _factory_preflight_report(
        selected_configs, module=module, scope="selected"
    )
    _print_factory_preflight_report(report, "selected ensemble")
    failures = report.loc[~report["ok"]].sort_values("model")
    if not failures.empty:
        lines = [
            "Selected ensemble contains model names that this exporter cannot instantiate.",
            "No final training was started. Add factories/aliases for these models first:",
        ]
        lines.extend(
            f"  - {row['model']}: {row['error_type']}: {row['error']}"
...
def _write_factory_report(reports: list[pd.DataFrame], out_path: Path) -> None:
    if not reports:
        return
    out_path = out_path.expanduser().resolve()
    out_path.parent.mkdir(parents=True, exist_ok=True)
    pd.concat(reports, ignore_index=True).to_csv(out_path, sep="\t", index=False)
    print(f"Factory preflight report written: {out_path}", flush=True)
...
def main():
    ...
    p.add_argument(
        "--experiment-dir",
        type=Path,
        default=Path("FINAL/experiments/IBD-2cohort"),
        help="Existing IBD two-cohort LAMPP experiment directory.",
    )
    p.add_argument(
        "--csv",
        type=Path,
        default=Path("data/lampp-newest/data/IBD/ibd_train.csv"),
        help="Full labelled IBD wide CSV used by the benchmark sweep.",
    )
    p.add_argument(
        "--out",
        type=Path,
    ...
    p.add_argument("--model-id", default="ibd_mpma_e", help="Stable backend model id.")
    p.add_argument(
        "--display-name",
        default="IBD MPMA-E",
    ...
    p.add_argument(
        "--target", default="IBD", help="Short target label for model lists."
    )
    p.add_argument(
        "--positive-value",
        default="1",
    ...
    p.add_argument(
        "--study-ids",
        default="",
        help="Comma-separated IBD cohort/study IDs used in the sweep. Empty string uses all rows in the CSV, matching STUDY_IDS=[] in the sweep.",
    )
    p.add_argument("--optimize-metric", default=DEFAULT_OPTIMIZE_METRIC)
    p.add_argument(
        "--preflight-only",
        action="store_true",
        help="Validate selected configs and model factories, then exit before data loading/training.",
    )
    p.add_argument(
        "--preflight-all-configs",
        action="store_true",
        help="Also scan every unique model name in the full configs registry. This is report-only; packaging fails only if selected models are missing.",
    )
    p.add_argument(
        "--factory-report-out",
        type=Path,
        default=None,
        help="Optional TSV path for the selected/registry factory preflight report.",
    )
    args = p.parse_args()
    ...
    factory_reports: list[pd.DataFrame] = []
    selected_factory_report = _preflight_selected_model_factories(
        selected_configs, module=module
    )
    factory_reports.append(selected_factory_report)

    if args.preflight_all_configs:
        registry_report = _factory_preflight_report(
            configs, module=module, scope="configs_registry"
        )
        _print_factory_preflight_report(registry_report, "full configs registry")
        factory_reports.append(registry_report)

    if args.factory_report_out is not None:
        _write_factory_report(factory_reports, args.factory_report_out)

    if args.preflight_only:
        print(
            "Preflight-only requested; exiting before CSV loading and final model training.",
            flush=True,
        )
        return
    ...
    manifest = {
        ...
        "model_type": "MPMA-E",
        "source": "LAMPP IBD two-cohort benchmark experiment outputs",
