...
def _load_module(path: Path | None):
    ...
    spec = importlib.util.spec_from_file_location("ibs_mpmae_model_defs", path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Cannot import sweep script: {path}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["ibs_mpmae_model_defs"] = module
...
def _read_inner_scores(
    ...
    files = sorted((experiment_dir / "inner_results").glob("outer_*__inner_*.parquet"))
    if not files:
        return {cid: 1.0 for cid in member_ids}
...
def _fit_superlearner_from_inner_oof(
    ...
    files = sorted(
        (experiment_dir / "inner_predictions").glob("outer_*__inner_*.parquet")
    )
    if not files:
        raise FileNotFoundError(
    ...
    idx_cols = [
        c for c in ["outer_study_id", "inner_study_id", "sample_id"] if c in df.columns
    ]
...
class IBSMPMAEDeploymentModel:
    ...
    def _profile_matrix_from_dataframe(
        ...
        # and wide style: rows=samples, columns=features. Wide benchmark CSVs may include a
        # sample_id/Name column; keep it as the returned sample identifier and exclude it
...
def main():
    ...
    p.add_argument(
        "--experiment-dir",
        type=Path,
        default=Path("FINAL/experiments/IBS"),
        help="Existing custom-script IBS experiment directory.",
    )
    p.add_argument(
        "--data-dir",
        type=Path,
        default=Path("data/IBS"),
        help="Directory with <study>_profiles.tsv and <study>_metadata.tsv.",
    )
    p.add_argument(
        "--out",
        type=Path,
        default=None,
        help="Output deployment zip. Default: ibs_mpma_e_model_<artifact-version>.zip",
    )
    p.add_argument(
        "--artifact-version",
        default="v0.1.0",
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional original IBS sweep script; used only to reuse _build_models when possible.",
    )
    p.add_argument(
        "--study-ids",
        default="",
        help="Comma-separated study IDs. Default: inferred from *_profiles.tsv.",
    )
    p.add_argument("--sample-id-col", default="Name")
    p.add_argument("--target-col", default="host_disease")
    p.add_argument("--positive-label", default="IBS")
    ...
    if args.study_ids.strip():
        study_ids = [x.strip() for x in args.study_ids.split(",") if x.strip()]
    elif module is not None and hasattr(module, "STUDY_IDS"):
        study_ids = [str(x) for x in module.STUDY_IDS]
    else:
        study_ids = _infer_study_ids(data_dir)
    ...
    for _, row in selected_configs.iterrows():
        ...
        X, y, sample_ids, schema = _build_member_training_matrix(
            arrays, study_ids, levels
        )
    ...
    manifest = {
        "artifact_schema": "mllabiome.ibs_mpmae_object.v1",
        "artifact_version": str(args.artifact_version),
        "model_id": "ibs_mpma_e",
        "display_name": "IBS MPMA-E",
        "description": "Locked IBS MPMA-E ensemble trained on the full labelled IBS dataset for deployment.",
        "target": "IBS",
        "model_type": "MPMA-E",
        "disease": "IBS",
        "source": "original/custom manuscript IBS LODO scripts",
        "experiment_dir": str(experiment_dir),
        "data_dir": str(data_dir),
        "trained_on_full_labelled_dataset": True,
        "performance_source": "existing LODO/custom-script outputs, not this full-data refit",
        ...
        "input_format": "MetaPhlAn-style TSV or wide sample-by-feature TSV/DataFrame",
    }
    model = IBSMPMAEDeploymentModel(
        trained_members,
    ...
    print(f"\nWrote deployment package: {args.out.expanduser().resolve()}")
    print(
        "This package is built from the original/custom results directory, not from the new mllabiome config API."
    )
