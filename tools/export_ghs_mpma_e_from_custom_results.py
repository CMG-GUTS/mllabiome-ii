...
def _filter_lampp_studies(
    ...
    if out.empty:
        raise ValueError(
            ...
            f"Check --study-ids against the GHS training CSV."
        )
    return out
...
def main():
    ...
    p.add_argument(
        "--experiment-dir",
        type=Path,
        default=Path("FINAL/experiments/GHS-LAMPP"),
        help="Existing GHS LAMPP experiment directory.",
    )
    p.add_argument(
        "--csv",
        type=Path,
        default=Path("data/lampp-newest/data/ghs/ghs_train.csv"),
        help="Full labelled GHS wide CSV used by the benchmark sweep.",
    )
    p.add_argument(
        "--out",
        type=Path,
    ...
    p.add_argument("--model-id", default="ghs_mpma_e", help="Stable backend model id.")
    p.add_argument(
        "--display-name",
        default="GHS MPMA-E",
    ...
    p.add_argument(
        "--target", default="GHS", help="Short target label for model lists."
    )
    p.add_argument(
        "--positive-value",
        default="1",
    ...
    p.add_argument(
        "--study-ids",
        default=",".join(str(x) for x in range(21, 75)),
        help="Comma-separated GHS study IDs used in the sweep. Empty string uses all rows in the CSV.",
    )
