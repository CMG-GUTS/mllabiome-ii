...
def _filter_lampp_studies(
    ...
    if out.empty:
        raise ValueError(
            ...
            f"Check --study-ids against the CRC training CSV."
        )
    return out
...
def main():
    ...
    p.add_argument(
        "--experiment-dir",
        type=Path,
        default=Path("FINAL/experiments/CRC-LAMPP"),
        help="Existing CRC LAMPP experiment directory.",
    )
    p.add_argument(
        "--csv",
        type=Path,
        default=Path("data/lampp-newest/data/crc/crc_train.csv"),
        help="Full labelled CRC wide CSV used by the benchmark sweep.",
    )
    p.add_argument(
        "--out",
        type=Path,
    ...
    p.add_argument("--model-id", default="crc_mpma_e", help="Stable backend model id.")
    p.add_argument(
        "--display-name",
        default="CRC MPMA-E",
    ...
    p.add_argument(
        "--target", default="CRC", help="Short target label for model lists."
    )
    p.add_argument(
        "--positive-value",
        default="1",
    ...
    p.add_argument(
        "--study-ids",
        default="2,3,10,11,13,16,18,19",
        help="Comma-separated CRC study IDs used in the sweep. Empty string uses all rows in the CSV.",
    )
